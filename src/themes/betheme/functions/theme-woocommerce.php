...
if (! function_exists('mfn_woo_styles')) {
	function mfn_woo_styles()
	{
		...
		wp_enqueue_script('mfn-woojs', get_theme_file_uri('/js/woocommerce.js'), false, time(), true);
...
function mfn_edit_tax_attr_form_fields ($tag) {
	...
	<tr class="form-field mfn-attr-image">
        ...
        <td><input type="hidden" id="mfn_tax_field" value="<?php echo $current_value; ?>" name="mfn_tax_field" class="<?php echo $field_name; ?>" required>
    ...
    <?php
		...
		<div class="form-field mfn-attr-image">
	        ...
	        <input type="hidden" id="mfn_tax_field" value="<?php echo $current_value; ?>" name="mfn_tax_field" class="<?php echo $field_name; ?>" required>
	        <?php if($current_obj->attribute_type == 'image'){
	        	$current_value = wp_get_attachment_url($current_value); ?>
				<div class="mfn-custom-img-container">
				    <img data-src="<?php echo $placeholder_url; ?>" src="<?php if ( $current_value ) : echo $current_value; else: echo $placeholder_url; endif; ?>" alt="" style="max-width:100%;" />
...
function mfn_get_woo_sidecart_content(){
	...
	foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
		...
		<div class="mfn-ch-product" data-row-key="<?php echo $cart_item_key; ?>" data-product-id="<?php echo $product_id; ?>">
...
function mfn_woo_ajax_add_to_cart_single() {
    ...
    $variation = $custom_data = array(); // Initializing
...
function mfn_new_badge_woo_product() {
	if(mfn_opts_get('product-badge-new') == 1){
		...
		if ( ( time() - ( 60 * 60 * 24 * $newness_days ) ) < $created ) {
		  echo '<span class="mfn-new-badge onsale-label onsale">' . esc_html__( 'NEW', 'woocommerce' ) . '</span>';
		}
	}
}
...
function mfn_get_per_page( $from_panel = false ){
	...
	if( isset($perpage) && !$from_panel ){
		return $perpage;
	}else if(isset($tmp_id) && is_numeric($tmp_id) && get_post_status($tmp_id) == 'publish' && get_post_type($tmp_id) == 'template' && null !== get_post_meta($tmp_id, 'mfn_template_perpage', true) ){
		// if is template
		return get_post_meta($tmp_id, 'mfn_template_perpage', true);
	}else{
		// theme options
...
add_action( 'wp_login_failed', 'mfn_wrong_login_red');
function mfn_wrong_login_red($username){
	$prmlnk = get_permalink( get_option('woocommerce_myaccount_page_id') );
	$referrer = $_SERVER['HTTP_REFERER'];
     if ( !empty($referrer) && !strstr($referrer,'wp-login') && !strstr($referrer,'wp-admin') && !strstr($referrer, $prmlnk) ) {
          wp_redirect( $prmlnk );
          exit;
     }
}
