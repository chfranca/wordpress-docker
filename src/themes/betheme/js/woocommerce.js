...
$(document).ready(function() {
  ...
  if( $('body').hasClass('mfn-ajax-add-to-cart') ){
    $( document ).on( 'click', '.single_add_to_cart_button:not(.disabled)', function(e) {
      ...
      var cc = $('#header_cart .header-cart-count').length ? $('#header_cart .header-cart-count').first().text() : 1;
  ...
  var productvariations = {
    run: function() {
      ...
      $('.mfn-variations-wrapper .mfn-vr-options li a').on('click', function(e) {
        ...
        $('.variations select#'+atr).val(id).trigger('change');*/
        productvariations.triggerChange();
      });
    ...
    start: function() {
        $('.wc_payment_methods .wc_payment_method').each(function() {
            if( !$(this).find('.mfn-payment-check').length ){
                $(this).append('<span class="mfn-payment-check"><i class="icon-check"></i></span>');
            }
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                $('.mfn-popup-quickview .mfn-quickview-slider').slick({
                    ...
                    dots: false,
                    arrows: true,
                    prevArrow: '<a class="slick-prev mfn-popup-arrow slick-arrow" href="#"><i class="icon-left-open-big"></i></a>',
                    nextArrow: '<a class="slick-next mfn-popup-arrow slick-arrow" href="#"><i class="icon-right-open-big"></i></a>',
                });
    ...
    refresh: function(){
      $.ajax({
        ...
        success: function(response){
          ...
          if(response.total > 0 || !$('body').hasClass('mfn-hidden-icon-count')){

              if($('.header-cart-count').length && $('.header-cart-total').length){
                $('.header-cart-count').text(response.total);
                $('.header-cart-total').html(response.sum);
              }else{
                $('.header-cart').append('<span class="header-cart-count">'+response.total+'</span>');
                $('#header_cart').append('<p class="header-cart-total">'+response.sum+'</p>');
              }
